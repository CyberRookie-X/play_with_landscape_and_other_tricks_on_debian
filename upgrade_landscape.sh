...
download_from_github_actions() {
  ...
  if [ -f "$LANDSCAPE_DIR/github_token" ]; then
    ...
    log "已从 $LANDSCAPE_DIR/github_token 读取GitHub Token"
  else
    log "警告: 未找到GitHub Token文件 $LANDSCAPE_DIR/github_token，可能无法下载Beta版本"
  fi
  ...
  if [ "$file_name" = "redirect_pkg_handler.sh" ]; then
    ...
    if ! download_with_retry "https://raw.githubusercontent.com/CyberRookie-X/Install_landscape_on_debian12_and_manage_compose_by_dpanel/main/redirect_pkg_handler.sh" "$output_file"; then
  ...
  case "$file_name" in
    "landscape-webserver-x86_64")
      artifact_name="landscape-webserver-x86_64"
      ;;
    "landscape-webserver-x86_64-musl")
      artifact_name="landscape-webserver-x86_64-musl"
      ;;
    "landscape-webserver-aarch64")
      artifact_name="landscape-webserver-aarch64"
      ;;
    "static.zip")
      artifact_name="static"
      ;;
    "redirect_pkg_handler-x86_64")
      artifact_name="redirect_pkg_handler-x86_64"
      ;;
    "redirect_pkg_handler-x86_64-musl")
      artifact_name="redirect_pkg_handler-x86_64-musl"
      ;;
    "redirect_pkg_handler-aarch64")
      artifact_name="redirect_pkg_handler-aarch64"
      ;;
    *)
  ...
  # 解析工作流运行ID
  workflow_run_id=$(echo "$response" | grep -o '"id":[0-9]*' | head -1 | cut -d: -f2)
  ...
  if command -v curl >/dev/null 2>&1; then
    artifacts_response=$(curl -s -H "Accept: application/vnd.github.v3+json" -H "$auth_header" "$artifacts_url")
  elif command -v wget >/dev/null 2>&1; then
    if [ -n "$GITHUB_TOKEN" ]; then
      artifacts_response=$(wget -qO- --header="Accept: application/vnd.github.v3+json" --header="$auth_header" "$artifacts_url")
    else
      artifacts_response=$(wget -qO- --header="Accept: application/vnd.github.v3+json" "$artifacts_url")
    fi
  fi
...
create_backup() {
  ...
  if command -v gzip >/dev/null 2>&1; then
    ...
    backup_file="$backup_dir/${backup_name}.bz2"
    log "使用压缩工具: bzip2"
    (cd "$temp_dir" && tar -cf - . | bzip2 -c > "$backup_file")
  elif command -v xz >/dev/null 2>&1; then
    backup_file="$backup_dir/${backup_name}.xz"
    log "使用压缩工具: xz"
...
cleanup_old_backups() {
  ...
  while IFS= read -r -d '' file; do
    backup_files+=($file)
  done < <(find "$backup_dir" -name "landscape-*.tar.gz" -o -name "landscape-*.tar.bz2" -o -name "landscape-*.tar.xz" -o -name "landscape-*.tar" -o -name "landscape-*.zip" -print0 | sort -rz)
...
upgrade_landscape_version() {
  ...
  local executable_filename=$(echo "$download_info" | cut -d'|' -f2)
  
  if ! download_with_retry "$download_url" "$temp_dir/$executable_filename"; then
    log "可执行文件下载失败"
    rm -rf "$temp_dir"
    exit 1
  fi
  ...
  redirect_pkg_handler_script_url=$(get_redirect_pkg_handler_url "$version_type" "script")
  ...
  # 根据安装目录中已有的文件下载redirect_pkg_handler二进制文件
  case "$SYSTEM_ARCH" in
    "x86_64")
      # 检查是否已存在x86_64版本
  ...
  if [ "$version_type" = "stable" ]; then
    ...
    chmod +x "$temp_dir/$executable_filename"
    
    # 获取当前版本信息
    local current_version=""
    if [ -f "$landscape_dir/$executable_filename" ]; then
      current_version=$("$landscape_dir/$executable_filename" --version 2>/dev/null)
    fi
    
    # 获取新版本信息
    local new_version=""
    new_version=$("$temp_dir/$executable_filename" --version 2>/dev/null)
  ...
  # 替换可执行文件
  local backup_file="$landscape_dir/$executable_filename.bak"
  if [ -f "$landscape_dir/$executable_filename" ]; then
    if ! mv "$landscape_dir/$executable_filename" "$backup_file"; then
      log "文件备份失败"
      control_landscape_service "start"
      rm -rf "$temp_dir"
      exit 1
  ...
  if ! mv "$temp_dir/$executable_filename" "$landscape_dir/$executable_filename"; then
    log "文件替换失败"
    # 尝试恢复备份
    if [ -f "$backup_file" ]; then
      mv "$backup_file" "$landscape_dir/$executable_filename"
    fi
    control_landscape_service "start"
    rm -rf "$temp_dir"
    exit 1
  ...
  if [ -f "$temp_dir/statictmp/index.html" ]; then
    static_source_dir="$temp_dir/statictmp"
  else
    # 在子目录中递归查找
    for dir in "$temp_dir/statictmp"/*/; do
  ...
  case "$SYSTEM_ARCH" in
    "x86_64")
      ...
      if [ "$USE_MUSL" = true ] && [ -f "$temp_dir/redirect_pkg_handler-x86_64-musl" ]; then
