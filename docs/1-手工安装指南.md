...
* 中国镜像加速不保证有效，如遇下载失败，再次执行脚本即可

* 无需自动重启系统，去除命令最后的 `r` 即可
...
curl -fsSL https://ghproxy.com/https://raw.githubusercontent.com/CyberRookie-X/Install_landscape_on_debian12_and_manage_compose_by_dpanel/main/upgrade_landscape.sh | bash -s upb cn r

```

```shell
# 升级到 稳定 版 并自动重启系统（中国镜像加速）
curl -fsSL https://ghproxy.com/https://raw.githubusercontent.com/CyberRookie-X/Install_landscape_on_debian12_and_manage_compose_by_dpanel/main/upgrade_landscape.sh | bash -s up cn r
