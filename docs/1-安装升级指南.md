...
<!-- [请使用 debian 13.0.0，其他版本 kernerl 存在兼容性问题，正在修复 ](https://github.com/ThisSeanZhang/landscape/issues/126)
Debian 13 iso 安装镜像: [官方下载](https://cdimage.debian.org/cdimage/archive/13.0.0/amd64/iso-dvd/debian-13.0.0-amd64-DVD-1.iso)  -->
[官方下载](https://cdimage.debian.org/debian-cd/current/amd64/iso-dvd/) | [兰州大学](https://mirror.lzu.edu.cn/debian-cd/current/amd64/iso-dvd/) | [163镜像](https://mirrors.163.com/debian-cd/current/amd64/iso-dvd/)   | [中国科学技术大学](https://mirrors.ustc.edu.cn/debian-cd/current/amd64/iso-dvd/) | [清华大学](https://mirrors.tuna.tsinghua.edu.cn/debian-cd/current/amd64/iso-dvd/) | [阿里云](https://mirrors.aliyun.com/debian-cd/current/amd64/iso-dvd/) | [上海交通大学](https://mirror.sjtu.edu.cn/debian-cd/current/amd64/iso-dvd/) | [浙江大学](https://mirrors.zju.edu.cn/debian-cd/current/amd64/iso-dvd/) | [南京大学](https://mirrors.zju.edu.cn/debian-cd/current/amd64/iso-dvd/) | [哈尔滨工业大学](https://mirrors.hit.edu.cn/debian-cd/current/amd64/iso-dvd/)
...
**debian用户：root，非 root 用户请自行添加 sudo**   
**注意❗**   
**语言选择 us/english，避免中文路径与某些软件不兼容,（后面会调整时区到上海）。**   
启用网络镜像站点，选择 中国 
建议选择 安装 sshserver、标准配置 
...
图片来自[链接](https://fanyops.com/art/67)
[Debian13 安装指南](https://fanyops.com/art/67)
...
* 使用 https://ghfast.top/ 下载安装脚本（wget）
```shell
# 安装 Landscape Router
...
wget https://raw.githubusercontent.com/CyberRookie-X/Install_landscape_on_debian12_and_manage_compose_by_dpanel/main/upgrade_landscape.sh -O upgrade_landscape.sh && chmod +x upgrade_landscape.sh && /bin/bash ./upgrade_landscape.sh --reboot --backup --stopdocker

```

```shell
# 升级到 稳定 版 ，备份当前版本，并自动重启系统（使用 curl）
curl -fsSL https://raw.githubusercontent.com/CyberRookie-X/Install_landscape_on_debian12_and_manage_compose_by_dpanel/main/upgrade_landscape.sh -o upgrade_landscape.sh && chmod +x upgrade_landscape.sh && /bin/bash ./upgrade_landscape.sh --reboot --backup --stopdocker 
...
wget https://ghfast.top/https://raw.githubusercontent.com/CyberRookie-X/Install_landscape_on_debian12_and_manage_compose_by_dpanel/main/upgrade_landscape.sh -O upgrade_landscape.sh && chmod +x upgrade_landscape.sh && /bin/bash ./upgrade_landscape.sh --cn --reboot --backup --stopdocker
...
curl -fsSL https://ghfast.top/https://raw.githubusercontent.com/CyberRookie-X/Install_landscape_on_debian12_and_manage_compose_by_dpanel/main/upgrade_landscape.sh -o upgrade_landscape.sh && chmod +x upgrade_landscape.sh && /bin/bash ./upgrade_landscape.sh --cn --reboot --backup --stopdocker
...
参数:
  ...
  --cn           - 使用中国镜像加速（可选）
  --reboot       - 升级完成后自动重启（可选）
  --stopdocker   - 替换 handler 时，先停止 Docker 服务（停止挂载了 handler 的容器，避免文件替换失败）
  --backup       - 升级前进行备份（可选）
...
```shell
# 升级到 Beta 版，备份当前版本，并自动重启系统（使用 wget）
wget https://raw.githubusercontent.com/CyberRookie-X/Install_landscape_on_debian12_and_manage_compose_by_dpanel/main/upgrade_landscape.sh -O upgrade_landscape.sh && chmod +x upgrade_landscape.sh && /bin/bash ./upgrade_landscape.sh --beta --reboot --backup --stopdocker

``` 

```shell
# 升级到 Beta 版，备份当前版本，并自动重启系统（使用 curl）
curl -fsSL https://raw.githubusercontent.com/CyberRookie-X/Install_landscape_on_debian12_and_manage_compose_by_dpanel/main/upgrade_landscape.sh -o upgrade_landscape.sh && chmod +x upgrade_landscape.sh && /bin/bash ./upgrade_landscape.sh --beta --reboot --backup --stopdocker

``` 

```shell
# 升级到 Beta 版，备份当前版本，并自动重启系统（使用 wget）从 github 镜像站下载脚本
wget https://ghfast.top/https://raw.githubusercontent.com/CyberRookie-X/Install_landscape_on_debian12_and_manage_compose_by_dpanel/main/upgrade_landscape.sh -O upgrade_landscape.sh && chmod +x upgrade_landscape.sh && /bin/bash ./upgrade_landscape.sh --beta --reboot --backup --stopdocker

``` 

```shell
# 升级到 Beta 版，备份当前版本，并自动重启系统（使用 curl）从 github 镜像站下载脚本
curl -fsSL https://ghfast.top/https://raw.githubusercontent.com/CyberRookie-X/Install_landscape_on_debian12_and_manage_compose_by_dpanel/main/upgrade_landscape.sh -o upgrade_landscape.sh && chmod +x upgrade_landscape.sh && /bin/bash ./upgrade_landscape.sh --beta --reboot --backup --stopdocker
...
[Service]
ExecStart=/root/.landscape-router/landscape-webserver-x86_64 -c /root/.landscape-router
# 注意这个路径与下面创建的landscape-router目录相同。 -c, --config-dir <CONFIG_DIR> 
Restart=always
User=root
